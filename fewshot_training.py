# -*- coding: utf-8 -*-
"""
完整训练脚本 - 基于原始小样本伪装目标分割模型

整合原始模型 + 改进方法：
1. 原始模型：FewShotCamouflageSegWithClass (原型匹配 + Cross-Attention)
2. 数据增强 (MixUp, CutMix)
3. 注意力机制 (CBAM, SE)
4. 高级损失函数 (Focal Loss, Tversky Loss)
5. 测试时增强 (TTA)
"""
import os
import sys
...
sys.path.insert(0, os.path.join(PROJECT_ROOT, 'my_models', '我的模型'))
...
class MaskedAvgPool(nn.Module):
    def forward(self, feat, mask):
        mask = F.interpolate(mask.unsqueeze(1) if mask.dim() == 3 else mask, size=feat.shape[-2:], mode='nearest')
        masked = feat * mask
        return masked.sum(dim=(2, 3)) / (mask.sum(dim=(2, 3)) + 1e-6)


class ProtoMatcher(nn.Module):
    def forward(self, feat, proto, aggregate='mean'):
        B, C, H, W = feat.shape
        ...
        proto = proto.view(1, C, 1).expand(B, -1, -1)
...
class C2FDecoder(nn.Module):
    def __init__(self, ch_c2=256, ch_c3=512, ch_c4=1024, ch_c5=2048, mid=256):
        ...
        self.up5 = nn.Sequential(
            nn.Conv2d(ch_c5, mid, 3, 1, 1),
            nn.BatchNorm2d(mid),
            nn.ReLU(inplace=True)
        )
        self.up4 = nn.Sequential(
            nn.Conv2d(ch_c4 + mid, mid, 3, 1, 1),
            nn.BatchNorm2d(mid),
            nn.ReLU(inplace=True)
        )
        self.up3 = nn.Sequential(
            nn.Conv2d(ch_c3 + mid, mid, 3, 1, 1),
            nn.BatchNorm2d(mid),
            nn.ReLU(inplace=True)
        )
        self.up2 = nn.Sequential(
            nn.Conv2d(ch_c2 + mid, mid, 3, 1, 1),
            nn.BatchNorm2d(mid),
            nn.ReLU(inplace=True)
        )
        ...
        self.guide_conv = nn.Conv2d(2, mid, 1)
    
    def forward(self, feats, guides):
        g = self.guide_conv(guides['g5'] if 'g5' in guides else guides.get('g4', torch.zeros(1, 2, 1, 1, device=feats['c5'].device)))
...
class FewShotSegModel(nn.Module):
    ...
    def build_guides(self, query_feats, protos):
        ...
        for lvl, gname in lvl_to_gname.items():
            ...
            guides[gname] = torch.cat([sim_fg - sim_bg, sim_fg], dim=1)
    ...
    def forward(self, support_imgs, support_masks, query_img):
        ...
        q_c4 = self.se(q_feats['c4'])
        class_logits = self.class_head(q_c4)
...
class DiceBCELoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        dice = 1 - (2 * inter + smooth) / (pred_sigmoid.sum() + target.sum() + smooth)
        return bce + dice
...
class CombinedSegLoss(nn.Module):
    ...
    def forward(self, pred, target):
        return 0.3 * self.focal(pred, target) + 0.3 * self.tversky(pred, target) + 0.4 * self.dice_bce(pred, target)
...
def train_seg_model(train_loader, val_loader, epochs=30, lr=1e-4):
    ...
    model = FewShotSegModel(num_classes=102, backbone='resnet50', pretrained=True).to(DEVICE)
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            seg_loss = seg_criterion(output['seg_logits'].squeeze(1), query_gts)
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                K = min(5, images.size(0) // 2)
                support_imgs = images[:K]
                support_masks = torch.zeros(K, images.size(2), images.size(3), device=DEVICE)
                query_imgs = images[K:]
                query_gts = gts[K:]
                ...
                preds = output['seg_prob'].cpu().numpy()
                preds = (preds > 0.5).astype(np.float32)
                
                for i in range(query_imgs.size(0)):
                    metrics = compute_seg_metrics(preds[i, 0], query_gts[i])
        ...
        if len(all_metrics) > 0:
            ...
            print(f"    Epoch {epoch+1}/{epochs}: Loss={train_loss/max(1,len(train_loader)):.4f}, mIoU={avg_metrics['mIoU']:.4f}, Dice={avg_metrics['Dice']:.4f}, F1={avg_metrics['F1']:.4f}")
        else:
            print(f"    Epoch {epoch+1}/{epochs}: Loss={train_loss/max(1,len(train_loader)):.4f}")
    
    return best_metrics if best_metrics else {'mIoU': 0, 'Dice': 0, 'Precision': 0, 'Recall': 0, 'F1': 0}
...
def main():
    print("="*80)
    print("完整训练实验 - 基于原始小样本伪装目标分割模型")
    print(f"设备: {DEVICE}")
    print(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*80)
    
    print("\n模型架构:")
    print("  - 原始FewShotCamouflageSeg模型")
    ...
    print("  2. 损失函数: Focal Loss, Tversky Loss, Dice-BCE")
    print("  3. 标签平滑")
    print("  4. 学习率调度: CosineAnnealingWarmRestarts")
    
    results = {
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'device': DEVICE,
        'model': 'FewShotCamouflageSeg + CBAM + SE + All Improvements',
        'experiments': {}
    }
    ...
    results['experiments']['COD10K_Segmentation_FewShot'] = seg_metrics
    ...
    results_file = os.path.join(RESULTS_DIR, 'fewshot_training_results.json')
